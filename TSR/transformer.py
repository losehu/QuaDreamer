...
class AxialAttention(nn.Module):
    def __init__(self, n_embd, n_head, attn_pdrop, resid_pdrop,
        ...
        h=20
        w=64
        self.row_rel_pos_bias = nn.Linear(2 * h - 1, self.n_head, bias=False)
        self.col_rel_pos_bias = nn.Linear(2 * w - 1, self.n_head, bias=False)

    def _cal_1d_pos_emb(self, hidden_states, rel_pos_onehot_size,h,w, row=True):
        ...
        if row:
            # row_rel_pos_bias = nn.Linear(2 * h - 1, self.n_head, bias=False)(rel_pos.device).to(rel_pos.dtype)
            rel_pos = self.row_rel_pos_bias(rel_pos).permute(0, 3, 1, 2)
        else:
            # col_rel_pos_bias = nn.Linear(2 * w - 1, self.n_head, bias=False).to(rel_pos.device).to(rel_pos.dtype)
    ...
    def forward(self, x, return_att=False, debug=False):  # x:[B,C,H,W], mask:[B,1,H,W]
        ...
        if self.add_rel_pos:
            row_rel_pos = self._cal_1d_pos_emb(x, rel_pos_onehot_size=h, h=h,w=w,row=True)
        else:
            row_rel_pos = None
        ...
        if self.add_rel_pos:
            col_rel_pos = self._cal_1d_pos_emb(x, rel_pos_onehot_size=w,h=h,w=w ,row=False)
        else:
            col_rel_pos = None
...
class EdgeLineGPTConfig:
    ...
    def __init__(self, **kwargs):
        for k, v in kwargs.items():
            setattr(self, k, v)
#/root/micromamba/envs/svd/lib/python3.10/site-packages/diffusers/models/attention.py
# init
        # # transformer, input: 32*32*config.n_embd

        # import sys
        # sys.path.append('/root/autodl-tmp/TrackDiffusion-SVD/TSR')
        # from transformer import BlockAxial,my_Block_2,EdgeLineGPTConfig
        # model_config = EdgeLineGPTConfig(embd_pdrop=0.0, resid_pdrop=0.0, n_embd=1024, block_size=32,
        #                                 attn_pdrop=0.0, n_layer=2, n_head=16)
        # self.TSR_Blocks = []
        # for _ in range(model_config.n_layer // 2):
        #     #self.TSR_Blocks.append(BlockAxial(model_config))
        #     self.TSR_Blocks.append(my_Block_2(model_config))
...
# forward
            #     hidden_states = block(
            #         hidden_states, #torch.Size([16, 1024, 320])
            #         encoder_hidden_states=encoder_hidden_states,#torch.Size([16, 1, 1024])
            #         cross_attention_kwargs=cross_attention_kwargs,
            #     ) #torch.Size([16, 1024, 320])  torch.Size([16, 256, 640]) torch.Size([16, 64, 1280])
            # if hidden_states.shape[1]==1024:
            #     hidden_states=hidden_states.view(hidden_states.shape[0], 1024, 16, 20)
            #     hidden_states=self.TSR_Blocks(hidden_states)        
            #     hidden_states=hidden_states.view(hidden_states.shape[0], 1024, 320)
            
            # hidden_states_mix = hidden_states #torch.Size([16, 1024, 320])
