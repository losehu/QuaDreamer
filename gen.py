...
if add_obj == 'true' or add_obj == 'True':
    ...
    add_obj = False 
# 加载预训练模型路径autodl-tmp/TrackDiffusion-SVD/outputs/
pretrained_model_path = "/root/autodl-tmp/TrackDiffusion-SVD/stable-video-diffusion-img2vid-xt"
# unet_path="/root/autodl-tmp/TrackDiffusion-SVD/stable-video-diffusion-img2vid-xt/unet"
...
unet = UNetSpatioTemporalConditionModel.from_pretrained(unet_path, torch_dtype=torch.float16,
                                                                low_cpu_mem_usage=False,      # 必须显式关掉 meta 加载
        ignore_mismatched_sizes=True)  # 关键：形状不对就随机初始化)
# 创建稳定视频扩散管道
...
def draw_bboxes_on_image(image, bboxes, colors):
    ...
    # 使用默认字体，避免某些系统找不到字体文件
    try:
        font = ImageFont.truetype("arial.ttf", 16)
    except IOError:
        font = ImageFont.load_default()

    for idx, (bbox, color) in enumerate(zip(bboxes, colors), start=0):
        x1, y1, x2, y2 = bbox
        draw.rectangle([x1, y1, x2, y2], outline=tuple(map(int, color)), width=3)
        draw.text((x1, y1 - 10), f"Box {idx}", fill=tuple(map(int, color)), font=font)
    
    return image

def draw_bboxes_and_save(ori_pixel_values, bboxes_prompt, output_dir,nn):
    """
    ...
    output_dir=os.path.join(output_dir,str(nn))
...
def validation_step(batch,nn):
    ...
    frames = []
    kkk=2
    for i in range(kkk,14):
        batch['bboxes'][i]=batch['bboxes'][kkk-1]
    images = pipe(
        ...
        imu_data= batch['imu'].unsqueeze(0) ,
        num_frames=14,
    ...
    # 定义从 PIL.Image 到 tensor 的转换
    transform = transforms.Compose([
        transforms.ToTensor(),                    # 转成 [0,1] 的 tensor，形状为 [C, H, W]
    ...
    # 拼接成一个 batch
    batch_tensor = torch.stack(tensor_list)  # shape: [14, 3, 128, 512]
        
    frames = []
    draw_bboxes_and_save(batch_tensor, batch['bboxes'], './box_pic',nn)
    
    for i in range(14):
        img = images[i].resize((2048, 480))
        img.save(batch['path'][i])
        print(f"保存图片到 {batch['path'][i]}")
...
def main(train_dataloader):
    ...
    for i in range(train_dataloader.__len__()):
        if i!=52:
            continue
        print(f"{i}/{train_dataloader.__len__()}")
        batch=train_dataloader.__getitem__(i)
        for j in range(len(batch['path'])):
            batch['path'][j]=os.path.join( unet_path.rstrip('/unet'),batch['path'][j].replace('./mydata','.'))
        validation_step(batch, i)

    # fvd_all=[]
    # for step, batch in enumerate(train_dataloader):
    #     print(f"{step+1}/{len(train_dataloader)}")
    #     validation_step(step,batch,fvd_all)
    #     with open(os.path.join(unet_path.rstrip('/unet') , f"eval_{timestamp}.txt"), 'a') as f:
    #         f.write(f"{step+1}/{len(train_dataloader)}: {sum(fvd_all) / len(fvd_all)}\n")
