...
from .layers import *
import os
add_mamba = os.getenv('ADD_MAMBA')
if add_mamba == 'true' or add_mamba == 'True':
    add_mamba = True
...
class LaMa_model(nn.Module):
    def __init__(self):
        ...
        # self.conv1 = nn.Conv2d(in_channels=5, out_channels=64, kernel_size=7, padding=0) #25f
        ...
        # self.convt4 = nn.Conv2d(in_channels=64, out_channels=5, kernel_size=7, padding=0) #25f
        self.convt4 = nn.Conv2d(in_channels=64, out_channels=4, kernel_size=7, padding=0)
...
class ReZeroFFC(LaMa_model):
    def __init__(self, config):
        ...
        self.config = config
        if add_mamba:
            self.add_mamba=SS2D(d_model=1280).to(dtype=torch.float16)
    def forward(self, x):
        # x = x.reshape(int(x.shape[0]/5), 5, x.shape[1], x.shape[2])#torch.Size([16, 16, 1280])->torch.Size([4, 4, 16, 1280]) 25f
        x = x.reshape(int(x.shape[0]/4), 4, x.shape[1], x.shape[2])#torch.Size([16, 16, 1280])->torch.Size([4, 4, 16, 1280])

        x = self.pad1(x)#torch.Size([4, 4, 16, 1280])->torch.Size([4, 4, 22, 1286])
        x = self.conv1(x)#torch.Size([4, 4, 22, 1286])->torch.Size([4, 64, 16, 1280])

        inp = x
        x = self.bn1(inp)#torch.Size([4, 64, 16, 1280])
        x = self.act(x)#orch.Size([4, 64, 16, 1280])
#mamba
        if add_mamba:
            B, A, P, C = x.shape
            x=self.add_mamba( x.view(B*A, C, 2, 8)).view(B, A, P, C)
        x = self.conv2(x )#torch.Size([4, 128, 8, 640])
        x = self.bn2(x)#torch.Size([4, 128, 8, 640])
        x = self.act(x)#torch.Size([4, 128, 8, 640])

        x = self.conv3(x )#torch.Size([4, 128, 8, 640])->torch.Size([4, 256, 4, 320])
        x = self.bn3(x)
        x = self.act(x)
