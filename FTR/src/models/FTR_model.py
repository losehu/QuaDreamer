...
from FTR.src.losses.adversarial import NonSaturatingWithR1
from FTR.src.losses.feature_matching import masked_l1_loss, feature_matching_loss
from FTR.src.losses.perceptual import ResNetPL
from FTR.src.models.LaMa import *
from FTR.src.models.TSR_model import *
from FTR.src.models.upsample import StructureUpsampling
from FTR.src.utils import get_lr_schedule_with_warmup, torch_init_model
...
class DefaultInpaintingTrainingModule(BaseInpaintingTrainingModule):
    ...
    def forward(self, batch):
        img = batch['image']
        masked_img = torch.cat([img, img], dim=1)
        batch['predicted_image'] = self.generator(masked_img)
        return batch['predicted_image']
    def process(self, batch):
        self.iteration += 1
        self.discriminator.zero_grad()
        # discriminator loss
    ...
    def discriminator_loss(self, batch):
        ...
        if self.config.AMP:
            ...
            batch = self(batch)
        batch[self.image_to_discriminator] = batch[self.image_to_discriminator]
        predicted_img = batch[self.image_to_discriminator].detach()
        discr_fake_pred, discr_fake_features = self.discriminator(predicted_img)
        fake_loss = self.adversarial_loss.discriminator_fake_loss(discr_fake_pred=discr_fake_pred, mask=batch['mask'])
        fake_loss.backward()
...
class Encoder(nn.Module):
    ...
    def forward(self, x, seq_lens):
        ...
        )  # (batch_size, frame_num, hidden_dim)
        
        latent = self.fc(lstm_out)  # (batch_size, frame_num, latent_dim)
        return latent

class Decoder(nn.Module):
    def __init__(self, latent_dim, output_channels, H, W):
        ...
        self.frame_decoder = nn.Sequential(
            ...
            nn.Unflatten(1, (512, 1, 1)),  # (bs*frame_num, 512, 1, 1)
            
            # Upsampling steps
            nn.ConvTranspose2d(512, 256, kernel_size=4, stride=2, padding=1),  # (batch_size*frame_num, 256, 2, 2)
            nn.ReLU(),
            nn.ConvTranspose2d(256, 128, kernel_size=4, stride=2, padding=1),  # (batch_size*frame_num, 128, 4, 4)
            nn.ReLU(),
            nn.ConvTranspose2d(128, 64, kernel_size=4, stride=2, padding=1),  # (batch_size*frame_num, 64, 8, 8)
            nn.ReLU(),
            nn.ConvTranspose2d(64, 32, kernel_size=4, stride=2, padding=1),   # (batch_size*frame_num, 32, 16, 16)
            nn.ReLU(),
            nn.ConvTranspose2d(32, 16, kernel_size=4, stride=2, padding=1),   # (batch_size*frame_num, 16, 32, 32)
            nn.ReLU(),
            nn.ConvTranspose2d(16, 8, kernel_size=4, stride=2, padding=1),    # (batch_size*frame_num, 8, 64, 64)
            nn.ReLU(),
            nn.ConvTranspose2d(8, 4, kernel_size=4, stride=2, padding=1),     # (batch_size*frame_num, 4, 128, 128)
            nn.ReLU(),
            nn.ConvTranspose2d(4, output_channels, kernel_size=(3, 4), stride=(1, 4), padding=(1, 0)),  # 修正参数
            nn.Sigmoid()
        )
        ...
        self.mask_generator = nn.Sequential(
            nn.Linear(latent_dim, H * W),  # Output size must match 128x512
            nn.Unflatten(1, (1, H, W)),    # Should match the dimensions of the frame output
            nn.Sigmoid()
        )

    def forward(self, latent):
        # latent: (batch_size, frame_num, latent_dim)
        batch_size, frame_num, _ = latent.shape
        
        # Flatten batch and frame dimensions
        latent = latent.view(-1, latent.size(-1))  # (batch_size*frame_num, latent_dim)
        
        # Generate image
        img = self.frame_decoder(latent)  # (batch_size*frame_num, 3, 128, 512)
        
        # Generate mask
        mask = self.mask_generator(latent)  # (batch_size*frame_num, 1, 128, 512)
        
        # Apply mask
        img = img * mask  # Now this should work because both have shape (batch_size*frame_num, 3, 128, 512)
        
        # Reshape back to (batch_size, frame_num, 3, H, W)
        img = img.view(batch_size, frame_num, 3, self.H, self.W)
        return img  # (batch_size, frame_num, 3, 128, 512)

class IMU2ImageNet(nn.Module):
    def __init__(self, input_dim=2, hidden_dim=64, latent_dim=128, 
                 output_channels=3, H=128, W=512):
        super(IMU2ImageNet, self).__init__()
        self.encoder = Encoder(input_dim, hidden_dim, latent_dim)
        self.decoder = Decoder(latent_dim, output_channels, H, W)

    def forward(self, x, seq_lens):
        latent = self.encoder(x, seq_lens)  # (batch, frame_num, latent_dim)
        output = self.decoder(latent)       # (batch, frame_num, 3, 128, 512)
        return output