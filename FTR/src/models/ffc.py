...
class FourierUnit(nn.Module):
    ...
    def forward(self, x):
        if torch.__version__ > '1.7.1' and '1.7.1' not in torch.__version__:
            ...
            # (batch, c, h, w/2+1, 2)
            fft_dim = (-2, -1)
            ffted = torch.fft.rfftn(x.to(dtype=torch.float32), dim=fft_dim, norm=self.fft_norm)
            ffted = torch.stack((ffted.real, ffted.imag), dim=-1)
            ffted = ffted.permute(0, 1, 4, 2, 3).contiguous()  # (batch, c, 2, h, w/2+1)
            ffted = ffted.view((batch, -1,) + ffted.size()[3:])
            ffted=ffted.to(dtype=torch.float16)
            ...
            output = torch.fft.irfftn(ffted.to(dtype=torch.float32), s=ifft_shape_slice, dim=fft_dim, norm=self.fft_norm)
