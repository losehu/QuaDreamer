...
def validation_step(step,batch,i3d,fvd_all):
    ...
    fvd = calculate_fvd(batch_tensor.unsqueeze(0).to("cuda"),tensor.unsqueeze(0).to("cuda") ,  torch.device("cuda"),i3d, method='styleganv', only_final=True,)
    # print(fvd)
...
def main(train_dataloader):
    ...
    for step, batch in enumerate(train_dataloader):
        ...
        for i in range(len(batch['path'])):
            batch['paths'].append(os.path.join( unet_path.rstrip('/unet'),batch['path'][i][0].replace('./mydata','.')))
        validation_step(step,batch,i3d,fvd_all)
